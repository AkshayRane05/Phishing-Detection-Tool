...
async def send_to_dashboard(email_data):
    """Send phishing email details to the dashboard via WebSocket and store in DB."""
    print("📨 Sending email to dashboard and DB:", email_data)
    uri = "ws://localhost:8000/ws"
    api_url = "http://localhost:8000/save_email"

    async with websockets.connect(uri) as websocket:
        await websocket.send(json.dumps(email_data))  # Send via WebSocket

    # Send email data to API to store in the database
    try:
        response = requests.post(api_url, json=email_data)
        print(f"📂 DB Save Response: {response.status_code}, {response.text}")
...
def predict_email(email_text):
    ...
    if prediction > 0.5:
        return "Phishing (Confidence: {:.2f}%)".format(prediction * 100)
    else:
        return "Legitimate (Confidence: {:.2f}%)".format((1 - prediction) * 100)
            ...
            if body:
                ...
                if "Phishing" in prediction:
                    ...
                    email_data = {
                        ...
                        "urls": urls  # Include extracted URLs
                    }

                    # Send data to WebSocket
                    # Send data to WebSocket
                    asyncio.run(send_to_dashboard(email_data))
